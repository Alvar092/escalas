...
struct TrunkControlTestView: View {
    
    @Environment(\.repositories) private var repositories
    
    let viewModel: TrunkControlTestViewModel
...
private struct TrunkControlTestContentView: View {
    let repositories: Repositories
    
    @Environment(\.navigationRouter) private var router
    @State var viewModel: TrunkControlTestViewModel
    ...
    @ViewBuilder
    private func optionButton(for option: TrunkControlScoreOption) -> some View {
        Button {
            ...
            .background(RoundedRectangle(cornerRadius: 8).fill(backgroundColorForOption(option)))
            .overlay(RoundedRectangle(cornerRadius: 8).stroke(strokeColorForOption(option)))
        }
    }
    
    private func backgroundColorForOption(_ option: TrunkControlScoreOption) -> Color {
...
#Preview {
    ...
    let test = TrunkControlTest.patient1
    router.currentTest = test
    
    return NavigationStack(path: .constant(NavigationPath())) {
        router.makeTestView(for: .trunkControlTest, repositories: repositories)
    }
